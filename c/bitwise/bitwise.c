...
int IsPower2NoLoop(unsigned int n) { return (n != 0) && (0 == (n & (n - 1))); }
...
{
    byte = ((byte & MASK_NIBBLE_UPPER) >> 4) |
           ((byte & for_groups_one_zeros) << 4);
...
void Swap(int* one,
          int* other) /* no temp variable */ /** make 3 different variations **/
{
    assert(NULL != one);
    assert(NULL != other);
