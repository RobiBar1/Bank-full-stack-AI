...
{
	return	BitArrSetBit(bitarr_t, index, 
	(0 != BitArrGetBit(bitarr_t, index)));
}
...
{
	size_t copy_word = word;
	
	copy_word = copy_word >> sizeof(word) * 8 - num_rotations;
	
	word = word << num_rotations;
	
	return word | num_rotations;
}
