...
static int ActiveFuncRoutine(scheduler_t* sc)
{
    ...
    if (sc->need_to_remove_self || FAILURE == status || DO_NOT_REPEAT == status)
    {
...
static int PriorityCmp(const void* left, const void* right)
{
    ...
    return TaskGetReadyTime((const task_t*)right) -
           TaskGetReadyTime((const task_t*)left);
}
...
void SchedulerRemoveTask(scheduler_t* sc, ilrd_uid_t uid)
{
    ...
    {
        task_to_remove =
            (task_t*)PQueueRemove(sc->pq, (const void*)&uid, CmpFunc);
...
ilrd_uid_t SchedulerAddTask(scheduler_t* sc, size_t time_interval,
                            task_func_t task_func, cleanup_func_t cleanup_func,
                            const void* param)
{
    task_t* task = NULL;
