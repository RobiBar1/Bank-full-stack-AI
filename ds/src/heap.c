...
static void HeapifyDownRecursive(heap_t* heap, size_t my_index,
                                 size_t capacity_last_index);
...
static void HeapifyUp(heap_t* heap, size_t index)
{
    ...
    parent = (0 >= index ? current
                         : VectorGetAccess(heap->vector, GET_PARENT(index)));
...
static size_t GetBigerIndex(heap_t* heap, size_t index_left, size_t index_right)
{
    ...
    if (0 < heap->cmp(*(void**)VectorGetAccess(heap->vector, index_left),
                      *(void**)VectorGetAccess(heap->vector, index_right)))
    {
        return index_left;
    }
...
static void CheckIfKeepHeapifyDown(heap_t* heap, size_t my_index,
                                   size_t other_inndex,
                                   size_t capacity_last_index)
{
    assert(NULL != heap);
    ...
    {
        Swap((void**)VectorGetAccess(heap->vector, my_index),
             (void**)VectorGetAccess(heap->vector, other_inndex));
        HeapifyDownRecursive(heap, other_inndex, capacity_last_index);
    }
}
                                 ...
                                 size_t capacity_last_index)
{
    ...
    if (capacity_last_index > index_right)
    {
        ...
        CheckIfKeepHeapifyDown(heap, my_index, bigger_side,
                               capacity_last_index);
...
void HeapPop(heap_t* heap)
{
    ...
    if (1 < VectorGetSize(heap->vector))
    {
        Swap(VectorGetAccess(heap->vector, 0),
             VectorGetAccess(heap->vector, VectorGetSize(heap->vector) - 1));
    }
                 ...
                 int (*is_match)(const void* data, const void* param))
{
    ...
    element_to_remove =
        Find(heap->vector, element_index, VectorGetSize(heap->vector),
             &element_index, data, is_match);
    if (NULL == element_to_remove)
    {
        return NULL;
    }

    last_index = VectorGetSize(heap->vector) - 1;
    ...
    *(void**)VectorGetAccess(heap->vector, element_index) =
        *(void**)VectorGetAccess(heap->vector, VectorGetSize(heap->vector) - 1);
    VectorPopBack(heap->vector);

    if (0 < element_index && element_index < VectorGetSize(heap->vector) &&
        0 < heap->cmp(*(void**)VectorGetAccess(heap->vector, element_index),
                      *(void**)VectorGetAccess(heap->vector,
                                               GET_PARENT(element_index))))
    {
