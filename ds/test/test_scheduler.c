...
/* gd test_scheduler.c ../src/scheduler.c ../src/pqueue.c  ../src/sorted_list.c
 * ../src/dlist.c ../src/task.c ../src/uid.c -I ../include/ */
...
void NothingFunction(void *param) { UNUSED(param); }
...
static test_status_t SchedulerRun_CheckCleanupBasicTest(void) {
  ...
  SchedulerAddTask(scheduler, 2, &PrintVenus, &PrintVenus_Cleanup,
                   &venus_malloc);
  SchedulerAddTask(scheduler, 3, &PrintMars, &PrintMars_Cleanup, NULL);
...
static test_status_t SchedulerRemoveTask_CheckRemoveSelfTest(void) {
  ...
  mercury_uid = SchedulerAddTask(scheduler, 1, &PrintMercury,
                                 &PrintMercury_Cleanup, mercury);
...
static test_status_t SchedulerRemoveTask_CheckRemoveOtherTest(void) {
  ...
  venus_uid = SchedulerAddTask(scheduler, 2, &PrintVenus, &PrintVenus_Cleanup,
                               &venus_malloc);
